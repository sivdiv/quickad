 ...
 * @Copyright (c) 2015-21 Devendra Katariya (bylancer.com)
 */
...
if (isset($_REQUEST['action'])){
    ...
    if ($_REQUEST['action'] == "payment_page_url") {payment_page_url();}
     if ($_REQUEST['action'] == "getPayment_page") {getPayment_page();}
}
...
function get_userMembership_by_id(){
    ...
    $results = get_user_membership_detail($user_id);
    send_json($results);
    die();
}
...
function get_membership_plan()
{
    ...
    if($plan['status']){
        if($plan['id'] == $sub_info['id']) {
            $sub_types['Selected'] = 1;
        } else {
            $sub_types['Selected'] = 0;
        }

        $sub_types['id'] = $plan['id'];
        $sub_types['title'] = $plan['name'];
        $sub_types['monthly_price'] = '0';
        $sub_types['annual_price'] = '0';
        $sub_types['lifetime_price'] = '0';

        $settings = $plan['settings'];
        $sub_types['limit'] = ($settings['ad_limit'] == "999")? $lang['UNLIMITED']: $settings['ad_limit'];
        $sub_types['duration'] = $settings['ad_duration'];
        $sub_types['featured_fee'] = $settings['featured_project_fee'];
        $sub_types['urgent_fee'] = $settings['urgent_project_fee'];
        $sub_types['highlight_fee'] = $settings['highlight_project_fee'];
        $sub_types['featured_duration'] = $settings['featured_duration'];
        $sub_types['urgent_duration'] = $settings['urgent_duration'];
        $sub_types['highlight_duration'] = $settings['highlight_duration'];
        $sub_types['top_search_result'] = $settings['top_search_result'];
        $sub_types['show_on_home'] = $settings['show_on_home'];
        $sub_types['show_in_home_search'] = $settings['show_in_home_search'];

        $sub_types['custom_settings'] = '';
        if(!empty($plan_custom)) {
            foreach ($plan_custom as $custom) {
                if(!empty($custom['title']) && trim($custom['title']) != '') {
                    $cs['custom_settings_id'] = $custom['id'];
                    $cs['custom_settings_title'] = $custom['title'];
                    $cs['custom_settings_value'] = '0';
                    if (isset($settings['custom'][$custom['id']]) && $settings['custom'][$custom['id']] == '1') {
                        $cs['custom_settings_value'] = '1';
                    }

                    $sub_types['custom_settings'] .= json_encode($cs);
                }
            }
        }
        $membership_plans[] = $sub_types;
    }
    ...
    foreach ($rows as $plan)
    {
        ...
        $sub_types['id'] = $plan['id'];
        $sub_types['title'] = $plan['name'];
        $sub_types['recommended'] = $plan['recommended'];

        $total_monthly += $plan['monthly_price'];
        $total_annual += $plan['annual_price'];
        $total_lifetime += $plan['lifetime_price'];

        $sub_types['monthly_price'] = $plan['monthly_price'];
        $sub_types['annual_price'] = $plan['annual_price'];
        $sub_types['lifetime_price'] = $plan['lifetime_price'];
...
/*
User Payment Page URL Api
action = payment_page_url
1. payment_type = "premium","subscr"
2. title  = "Payment Title"
3. psot_id
4. featured   "1" or "0"
5. urgent   "1" or "0"
6. highlight   "1" or "0"
...
Messages
1. url : Payment Page Secure url
*/
//test link
// http://localhost/quickad/api/v1/index.php?action=payment_page_url&payment_type=premium&title=adtitle&amount=10&post_id=3396&featured=1&urgent=1&highlight=0
function payment_page_url(){
    global $link,$lang;
    ...
    if (checkloggedin()) {
        ...
        $status = "error";
        $message = "Session Expire need to  log_in again";
    }
...
/*
User Payment Page URL Api
action = getPayment_page
1. username
2. password
3. payment_type = "premium","subscr"
...
Messages
On success its redirect to payment page
1. url : Payment Page Secure url
2. Error : Username or Password not found
*/
//test link
// http://localhost/quickad/api/v1/index.php?action=getPayment_page&username=demo&password=demo&payment_type=premium&title=adtitle&amount=10&post_id=3396&featured=1&urgent=1&highlight=0
function getPayment_page(){
    global $link,$lang;
    ...
    {
        $results['status'] = "error";
        $results['message'] = $lang['ACCOUNTBAN'];
    ...
    if($status == "success"){
        ...
        if($payment_type == 'premium'){
            ...
            /*These details save in session and get on payment success*/
    ...
    if ($rows > 0) {
        foreach($result as $info) {
            ...
            if(check_user_upgrades($info['user_id']))
            {
                ...
                $item['subcription_title'] = $sub_info['name'];
                $item['subcription_image'] = $sub_info['badge'];
            }else{
...
function ad_detail(){
    global $config,$link,$lang,$results;
    ...
    if(isset($_REQUEST['item_id'])){
        ...
        if ($num_rows > 0) {
            ...
            $item['product_url'] = $link['POST-DETAIL'].'/' . $info['id'] . '/'.$pro_url;
            $custom_data = array();
            ...
            foreach ($rows as $customdata){
                ...
                $pro_url = create_slug($info['product_name']);
                $item['page_link']  = $link['POST-DETAIL'].'/' . $info['id'] . '/'.$pro_url;
        ...
        if(isset($_REQUEST['custom'])){
            foreach ($custom_fields as $key => $value) {
                if ($value['userent']) {
                    $cf['id'] = $value['id'];
                    $cf['type'] = $value['type'];
                    if(isset($cf['textarea']) && $cf['textarea'] == "textarea")
...
function upload_product_picture(){
    ...
    $getAvatar = fileUpload($path_avatar, $file_avatar, "image", $first_title, 600, 600,true);
...
function payment_api_detail_config()
{
    ...
    /******************Flutterwave*******************/
    $results['flutterwave_api_key'] = $config['flutterwave_api_key'];
    $results['flutterwave_secret_key'] = $config['flutterwave_secret_key'];
...
function payment_success_saving(){
    global $config,$lang,$link,$mysqli;
    $pdo = ORM::get_db();
    //$lang_code = isset($_REQUEST['lang_code']) ? $_REQUEST['lang_code'] : null;
    $title = $_REQUEST['name'];
    $amount = $_REQUEST['amount'];
    ...
    if($folder == 'wire_transfer'){
        ...
        if($payment_type == "subscr") {
            ...
            $plan_interval = $_REQUEST['plan_interval'];
