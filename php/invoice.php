    ...
    if (isset($_SESSION['user']['id']) || isset($_SESSION['admin']['id'])) {
        ...
        foreach ($taxes as $tax){
            ...
            if($tax['type'] == 'inclusive'){
                ...
                $plan_taxes[$tax['id']]['value_formatted'] = price_format($inclusive_tax, $currency_code);
            }
        ...
        foreach ($taxes as $tax){
            if($tax['type'] == 'exclusive'){
                ...
                $plan_taxes[$tax['id']]['value_formatted'] = price_format($exclusive_tax, $currency_code);
            }
        }

        $premium = '';
        if($rows['transaction_method'] == 'Subscription'){
            $premium = $lang['MEMBERSHIP'];
        }else{
            ...
            if ($featured == "1") {
                $premium = $premium .' '. $lang['FEATURED'];
            }

            if ($urgent == "1") {
                $premium = $premium .' '. $lang['URGENT'];
            }

            if ($highlight == "1") {
                $premium = $premium .' '. $lang['HIGHLIGHT'];
            }
        }
        ...
        $page->SetParameter('ITEM_NAME', $rows['product_name'].'<br>'.$premium);
        $page->SetParameter('PAID_VIA', $payment_gateway['payment_title']);
        $page->SetParameter('ITEM_AMOUNT', price_format($price_without_inclusive,$currency_code));
        $page->SetParameter('TOTAL_AMOUNT', price_format($rows['amount'],$currency_code));
