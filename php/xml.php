...
if($config['xml_latest'] == 1){
    ...
    foreach ($rows as $info)
    {
        ...
        $item_created_at = date('Y-m-dTH:i:sP', strtotime($info['created_at']));
        $item_updated_at = date('Y-m-dTH:i:sP', strtotime($info['updated_at']));
        echo '<url>';
    ...
    foreach ($rows as $info)
    {
        ...
        $created_at = date('Y-m-dTH:i:sP', strtotime($info['created_at']));
        $updated_at = date('Y-m-dTH:i:sP', strtotime($info['updated_at']));
        echo '<url>';
    ...
    $rows = ORM::for_table($config['db']['pre'].'blog')
        ->select_many('id','title','created_at')
