...
function checkloggedadmin(){
    ...
    elseif(isset($_COOKIE["quickadmin_remember"]))
    {
        // Get the user_id from cookie.
        $userdata = $_COOKIE["quickadmin_remember"];

        $regex = '/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/';

        if(!preg_match("/^[[:alnum:]]+$/", $userdata))
        {
            if(!preg_match($regex,$userdata))
            {
                return false;
            }
            else{
        ...
        $result = ORM::for_table($config['db']['pre'].'admins')
            ->select_many('id','username','password_hash','permission')
            ->where($where, $userdata)
            ->find_one();
