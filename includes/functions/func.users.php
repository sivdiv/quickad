...
function checkloggedin()
{
    ...
    elseif(isset($_COOKIE["quickad_remember_me"]))
    {
        // Get the user_id from cookie.
        $user_id = $_COOKIE["quickad_remember_me"];

        $result = ORM::for_table($config['db']['pre'].'user')
            ->select_many('username','password_hash')
            ->where('id', $user_id)
            ->find_one();

        if (!empty($result)) {
            // Logged In!!!!
            create_user_session($user_id,$result['username'],$result['password_hash']);
            return true;
        } else {
            // Not logged in
            return false;
